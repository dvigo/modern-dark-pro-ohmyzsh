...
# Supports both 'standard' and 'capsule' styles dynamically.
...
# Theme style configuration (standard or capsule)
MODERN_DARK_PRO_STYLE="${MODERN_DARK_PRO_STYLE:-standard}"
...
if [[ "${COLORTERM}" == "truecolor" || "${COLORTERM}" == "24bit" ]]; then
  ...
  if [[ "${MODERN_DARK_PRO_VARIANT}" == "monokai" ]]; then
    ...
    if [[ "${MODERN_DARK_PRO_STYLE}" == "capsule" ]]; then
...
# Configuration Variables with defaults based on style
if [[ "${MODERN_DARK_PRO_STYLE}" == "capsule" ]]; then
  MODERN_DARK_PRO_NERD_FONTS="${MODERN_DARK_PRO_NERD_FONTS:-true}"
  MODERN_DARK_PRO_PATH_STYLE="${MODERN_DARK_PRO_PATH_STYLE:-limit}"
  MODERN_DARK_PRO_PROMPT_LAYOUT="${MODERN_DARK_PRO_PROMPT_LAYOUT:-two-line}"
else
  MODERN_DARK_PRO_NERD_FONTS="${MODERN_DARK_PRO_NERD_FONTS:-false}"
  MODERN_DARK_PRO_PATH_STYLE="${MODERN_DARK_PRO_PATH_STYLE:-shrink}"
  MODERN_DARK_PRO_PROMPT_LAYOUT="${MODERN_DARK_PRO_PROMPT_LAYOUT:-classic}"
fi
...
# Capsule and layout customizations
...
_MODERN_DARK_PRO_GIT_DIRTY=0
_MODERN_DARK_PRO_GIT_STAGED=0
_MODERN_DARK_PRO_GIT_UNTRACKED_ANY=0
_MODERN_DARK_PRO_GIT_UNTRACKED_COUNT=0
_MODERN_DARK_PRO_GIT_STASHED=0
...
function _modern_dark_pro_update_git() {
  ...
  if [[ -n "${file_lines}" ]]; then
    while IFS= read -r line; do
      ...
      local code="${line:0:2}"
      if [[ "${code}" == "??" ]]; then
        _MODERN_DARK_PRO_GIT_UNTRACKED_ANY=1
        (( _MODERN_DARK_PRO_GIT_UNTRACKED_COUNT++ ))
      else
        local index_char="${line:0:1}"
        local work_char="${line:1:1}"
        
        if [[ "${index_char}" != " " && "${index_char}" != "?" ]]; then
          _MODERN_DARK_PRO_GIT_STAGED=1
        fi
        if [[ "${work_char}" != " " && "${work_char}" != "?" ]]; then
          _MODERN_DARK_PRO_GIT_DIRTY=1
        fi
      fi
    done <<< "${file_lines}"
  fi
  ...
  if git rev-parse --verify refs/stash &>/dev/null; then
    _MODERN_DARK_PRO_GIT_STASHED=1
  fi

  # Only run git diff shortstat if in capsule style (saves an extra process for standard)
  _MODERN_DARK_PRO_GIT_FILES=0
  _MODERN_DARK_PRO_GIT_INSERTIONS=0
  _MODERN_DARK_PRO_GIT_DELETIONS=0
  if [[ "${MODERN_DARK_PRO_STYLE}" == "capsule" ]]; then
...
function _modern_dark_pro_path() {
  local style="${MODERN_DARK_PRO_PATH_STYLE}"
  local depth="${MODERN_DARK_PRO_PATH_DEPTH:-3}"
  
  if [[ "${style}" == "shrink" ]]; then
    local path_str="%~"
    # Expand %~ first to resolve Zsh prompt mapping
    path_str="${(%%)path_str}"
...
# ------------------------------------------------------------------------------
# STYLE 1: Standard Mode Layout & Formatting Functions
# ------------------------------------------------------------------------------
...
# Standard Git Status Prompt (rendered from cached variables)
function _modern_dark_pro_git_prompt_standard() {
  ...
  [[ $_MODERN_DARK_PRO_GIT_DIRTY -eq 1 ]] && status_items+=("%F{${COLOR_ERROR}}${MODERN_DARK_PRO_DIRTY_SYMBOL}%f")
  [[ $_MODERN_DARK_PRO_GIT_STAGED -eq 1 ]] && status_items+=("%F{${COLOR_SUCCESS}}${MODERN_DARK_PRO_STAGED_SYMBOL}%f")
  [[ $_MODERN_DARK_PRO_GIT_UNTRACKED_ANY -eq 1 ]] && status_items+=("%F{${COLOR_WARNING}}${MODERN_DARK_PRO_UNTRACKED_SYMBOL}%f")
  [[ $_MODERN_DARK_PRO_GIT_STASHED -eq 1 ]] && status_items+=("%F{${COLOR_WARNING}}${MODERN_DARK_PRO_STASHED_SYMBOL}%f")
  
  [[ $_MODERN_DARK_PRO_GIT_AHEAD -gt 0 ]] && status_items+=("%F{${COLOR_SUCCESS}}${MODERN_DARK_PRO_AHEAD_SYMBOL}${_MODERN_DARK_PRO_GIT_AHEAD}%f")
  [[ $_MODERN_DARK_PRO_GIT_BEHIND -gt 0 ]] && status_items+=("%F{${COLOR_ERROR}}${MODERN_DARK_PRO_BEHIND_SYMBOL}${_MODERN_DARK_PRO_GIT_BEHIND}%f")
  
  local git_url=""
  if [[ "${MODERN_DARK_PRO_CLICKABLE_GIT}" == "true" && -n "${_MODERN_DARK_PRO_CACHED_GIT_REMOTE}" && -z "${SSH_CONNECTION}" && -z "${SSH_CLIENT}" && -z "${SSH_TTY}" ]]; then
    local REPLY
    _modern_dark_pro_urlencode "${_MODERN_DARK_PRO_GIT_BRANCH}"
  ...
  local git_display
  if [[ -n "${git_url}" ]]; then
    git_display="%{\e]8;;${git_url}\e\\%}%F{${COLOR_GIT_BRANCH}}${MODERN_DARK_PRO_GIT_SYMBOL} ${_MODERN_DARK_PRO_GIT_BRANCH}%f%{\e]8;;\e\\%}"
  else
    git_display="%F{${COLOR_GIT_BRANCH}}${MODERN_DARK_PRO_GIT_SYMBOL} ${_MODERN_DARK_PRO_GIT_BRANCH}%f"
  fi
...
function _modern_dark_pro_elapsed_time() {
  if [[ -n "${_MODERN_DARK_PRO_ELAPSED_TIME_VAL}" ]]; then
    echo -n " %F{${COLOR_EXEC_TIME}}${MODERN_DARK_PRO_TIME_SYMBOL} ${_MODERN_DARK_PRO_ELAPSED_TIME_VAL}%f"
  fi
}


# ------------------------------------------------------------------------------
# STYLE 2: Capsule Mode Layout & Formatting Functions
# ------------------------------------------------------------------------------

# Renders a segment inside a capsule/pill (for Capsule style)
...
function _modern_dark_pro_git_changes_capsule() {
  ...
  local total_files=$(( _MODERN_DARK_PRO_GIT_FILES + _MODERN_DARK_PRO_GIT_UNTRACKED_COUNT ))
  ...
  if [[ ${_MODERN_DARK_PRO_GIT_UNTRACKED_COUNT} -gt 0 && ${_MODERN_DARK_PRO_GIT_FILES} -eq 0 ]]; then
    content+=" %F{${COLOR_WARNING}}?${_MODERN_DARK_PRO_GIT_UNTRACKED_COUNT}%f"
  fi
...
# Renders all active elements/capsules based on the current style (standard vs capsule)
function _modern_dark_pro_prompt_content() {
  if [[ "${MODERN_DARK_PRO_STYLE}" == "capsule" ]]; then
    local -a capsules
    local cap
    ...
    # Standard mode formatting: raw colored text segments
    local path_block="%F{${COLOR_PATH}}${_MODERN_DARK_PRO_PATH_VAL}%f"
    if [[ -n "${_MODERN_DARK_PRO_PATH_URL}" ]]; then
      path_block="%{\e]8;;${_MODERN_DARK_PRO_PATH_URL}\e\\%}${path_block}%{\e]8;;\e\\%}"
    fi
    
    echo -n "${_MODERN_DARK_PRO_SSH_STATUS} ${path_block}${_MODERN_DARK_PRO_READONLY}${_MODERN_DARK_PRO_GIT_STATUS}${_MODERN_DARK_PRO_VENV}${_MODERN_DARK_PRO_NODE}${_MODERN_DARK_PRO_GO}${_MODERN_DARK_PRO_RUST}${_MODERN_DARK_PRO_TF}${_MODERN_DARK_PRO_K8S}${_MODERN_DARK_PRO_AWS}${_MODERN_DARK_PRO_JOBS}${_MODERN_DARK_PRO_ELAPSED_TIME}"
  fi
}
...
function _modern_dark_pro_first_line() {
  local left
  if [[ "${MODERN_DARK_PRO_STYLE}" == "capsule" ]]; then
    left="%F{${COLOR_CONNECTOR}}┌─%f $(_modern_dark_pro_prompt_content)"
  else
    left="%F{${COLOR_CONNECTOR}}┌─%f$(_modern_dark_pro_prompt_content)"
  fi
  ...
  if [[ "${MODERN_DARK_PRO_SHOW_CLOCK}" == "true" ]]; then
    right="%F{${COLOR_CONNECTOR}}${MODERN_DARK_PRO_TIME_SYMBOL} %D{%H:%M:%S}%f"
    local expanded_right="${(%%)right}"
    local clean_right="${expanded_right//$'\x1b'\[[0-9;]##[a-zA-Z]/}"
    right_width=${(m)#clean_right}
  fi
...
function _modern_dark_pro_first_line_two_line() {
  local left="$(_modern_dark_pro_prompt_content)"
  # If standard mode, it has a leading space. Strip it for two-line layout so it is left-aligned perfectly.
  if [[ "${MODERN_DARK_PRO_STYLE}" != "capsule" ]]; then
    left="${left# }"
  fi
...
# Renders the content of a single-line prompt dynamically
function _modern_dark_pro_single_line_content() {
  local content="$(_modern_dark_pro_prompt_content)"
  if [[ "${MODERN_DARK_PRO_STYLE}" != "capsule" ]]; then
    content="${content# }"
  fi
  echo -n "${content}"
...
${_MODERN_DARK_PRO_PROMPT_CHAR} '
    ...
    PROMPT='$(_modern_dark_pro_single_line_content) ${_MODERN_DARK_PRO_PROMPT_CHAR} '
    if [[ "${MODERN_DARK_PRO_SHOW_CLOCK}" == "true" ]]; then
...
function _modern_dark_pro_precmd() {
  ...
  # Set up precomputed standard prompt strings
  _MODERN_DARK_PRO_SSH_STATUS=$(_modern_dark_pro_ssh_status)
  _MODERN_DARK_PRO_READONLY=$(_modern_dark_pro_readonly)
  _MODERN_DARK_PRO_GIT_STATUS=$(_modern_dark_pro_git_prompt_standard)
  ...
  _MODERN_DARK_PRO_ELAPSED_TIME=$(_modern_dark_pro_elapsed_time)
